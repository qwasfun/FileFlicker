...
export default function VideoPlayer() {
  ...
  const { data: subtitles = [] } = useQuery<Array<{ path: string; name: string; language: string }>>({
    queryKey: ["/api/files", id, "subtitles"],
    enabled: !!id && file?.type === "video",
  });
        ...
        <div className="bg-black/80 p-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <Select value={quality} onValueChange={setQuality}>
              <SelectTrigger data-testid="select-quality" className="bg-white/20 text-white border-white/30 w-24">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <div className="text-white text-sm">
                  ...
                  <span data-testid="text-duration" className="ml-1">{formatDuration(file.duration)}</span>
                </div>
